...
public class AuthorizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (ac == null) {
            // Người dùng chưa đăng nhập, hãy chuyển hướng đến trang đăng nhập
            HttpServletResponse httpResponse = (HttpServletResponse) response;
            httpResponse.sendRedirect("Login");
            return;
        }
        if (userHasPermission(ac.userName,ac.password,ac.role)) {
    ...
    private boolean userHasPermission(String userName,String password,String Role) {
        LoginDAO dao = new LoginDAO();
        if (dao.isAdmin(userName, password, Role)) {
            return true;
        }
        return false;
    }
